...
?>
	<div id="sidebar" >
		<ul class="snav">
			<li id="sb_upload" ><a href="upload.php" <?php check_menu('upload');  ?>><?=$tr('FILE_MANAGEMENT');?></a></li>
<?php if(isset($_GET['i']) && $_GET['i'] != '') { ?>
			<li id="sb_image" ><a href="#" class="current"><?php i18n('IMG_CONTROl_PANEL');?></a></li><?php }?>
<?php event::create("files-sidebar");
if (!getDef('GSNOUPLOADIFY',true)) { ?>	
			<li class="upload" id="sb_uploadify" >
				<div id="uploadify"></div><?php 
...
<?php } ?>
			<li style="float:right;" id="sb_filesize" ><small><?php i18n('MAX_FILE_SIZE'); ?>: <strong><?=(toBytes(ini_get('upload_max_filesize'))/1024)/1024; ?>MB</strong></small></li>
		</ul>
<?php 
if (getDef('GSNOUPLOADIFY',true)) { ?>
		<form class="uploadform" action="upload.php?path=<?php echo $path; ?>" method="post" enctype="multipart/form-data">
			...
			<input type="submit" class="submit" name="submit" value="<?=$tr('UPLOAD');?>" />
		</form>
<?php } else { ?>
